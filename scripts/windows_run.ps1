# 穹盾智矿 Windows 一键启动脚本
# 这个文件必须保存为 UTF-8 with BOM，否则中文 PowerShell 会乱码
$ErrorActionPreference = "Stop"
$root = Split-Path -Parent (Split-Path -Parent $PSCommandPath)
Set-Location $root

# 让 PowerShell 控制台正确显示 UTF-8 中文
...
if (-not $py) {
    ...
    Write-Host ""
    Write-Host "    请先下载并安装 Python 3.11:" -ForegroundColor Yellow
...
if (-not $node) {
    ...
    Write-Host ""
    Write-Host "    请先下载并安装 Node.js 20 LTS:" -ForegroundColor Yellow
...
if (-not (Test-Path "backend_service\venv\Scripts\python.exe")) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "[X] 后端依赖安装失败" -ForegroundColor Red
        Read-Host "按回车退出"
        exit 1
    }
...
if (-not (Test-Path "venv\Scripts\python.exe")) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "[X] AI 依赖安装失败" -ForegroundColor Red
        Read-Host "按回车退出"
        exit 1
    }
...
if (-not (Test-Path "frontend_dashboard\node_modules")) {
    ...
    Pop-Location
    if ($rc -ne 0) {
        Write-Host "[X] 前端依赖安装失败" -ForegroundColor Red
        Read-Host "按回车退出"
        exit 1
    }
...
$cmdBiz    = "& '" + $root + "\backend_service\venv\Scripts\uvicorn.exe' business_api:app --host 127.0.0.1 --port 8002"
$cmdSensor = "`$env:DEMO_MODE='true'; & '" + $root + "\backend_service\venv\Scripts\python.exe' backend_service\api_server.py"
$cmdAI     = "& '" + $root + "\venv\Scripts\python.exe' backend_service\ai_prediction_engine.py"
$cmdVision = "& '" + $root + "\venv\Scripts\python.exe' ai_core\vision_inference_service.py"
$cmdFront  = "npm run dev -- --host 127.0.0.1"

Start-Svc "business_api" 8002 "$root\backend_service"   $cmdBiz
Start-Svc "sensor_api"   8000 "$root"                   $cmdSensor
Start-Svc "ai_engine"    8001 "$root"                   $cmdAI
Start-Svc "vision_api"   8003 "$root"                   $cmdVision
Start-Svc "frontend"     5173 "$root\frontend_dashboard" $cmdFront
