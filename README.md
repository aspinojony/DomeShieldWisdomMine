...
- **多服务后端结构**：拆分监测 API、AI 推演、业务 API、视觉识别服务，便于独立开发和演示联调。
- **AI/视觉接入**：集成 YOLOv8 裂缝识别服务，支持上传图片、输出识别框、告警等级和历史记录。
- **前端可视化**：Vue3 + ECharts + Cesium 构建矿山态势大屏、智能识别页和生产运营工作台。
- **工程化部署**：提供 Docker Compose、启动脚本、检查脚本和 Windows 一键启动入口。
